...
def main():
    ...
    vod = 'vendor_' + args.file + '_strace.txt'  # output json file path
    ...
    command = 'strace -v -s 100 ./%s 2> %s' % (if_path, vod_path)
    print('[+] Executing %s ...' % command)
    process = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True)

    # strace output file stream
    strace_ifs = open(vod_path, "r")  # strace input filestream
    ...
    ojf_path = 'vendor_' + args.file + '_od.json'  # output json file path
    vod_path2 = ojf_path.replace('./', '')
    with open(vod_path2, "w") as od_json:
    ...
    with open(vod_path2, 'r') as od_json:
        for line in od_json:
            js = json.loads(line)
            key = list(js.keys())[0]
    ...
    command2 = 'echo : %s >> %s' % (root, vod_path2)
    process2 = subprocess.Popen(command2, stdout=subprocess.PIPE, shell=True)
    print("Open declaration generated - " +  vod_path2)
